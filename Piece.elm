...
type Piece = Piece Shape Orientation
...
getShape (Piece shape orientation) =
  case shape of
    ...
    ZShape ->
      if isVertical orientation
        then verticalZShape
        else horizontalZShape

render : Piece -> Form
render piece = drawShape <| getShape piece

getWidth : Piece -> Int
getWidth (Piece shape orientation) =
  case shape of
    IShape ->
      if isVertical orientation then 1 else 3
    JShape ->
      if isVertical orientation then 2 else 3
    LShape ->
      if isVertical orientation then 2 else 3
    OShape -> 2
    SShape ->
      if isVertical orientation then 3 else 2
    TShape ->
      if isVertical orientation then 3 else 2
    ZShape ->
      if isVertical orientation then 3 else 2
...
rotate (Piece shape orient) =
  Debug.log "rotation" <| case orient of
    North -> Piece shape East
    East -> Piece shape South
    South -> Piece shape West
    West -> Piece shape North

getBlocks : Piece -> List (Int, Int)
getBlocks piece =
  let shape = getShape piece
  in List.concat <| List.concat <| List.indexedMap (\y row -> List.indexedMap (\x present -> if present then [(x, y)] else [] ) row) shape
